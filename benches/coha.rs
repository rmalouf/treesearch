...
fn tree_iter_ordered_single(bencher: Bencher) {
    bencher.bench_local(|| {
        let treebank = Treebank::from_path(Path::new("examples/text_fic_2010.conll"));
        let count = black_box(treebank.trees(true).count());
        black_box(count);
    });
}
 */
...
fn tree_iter_unordered_single(bencher: Bencher) {
    bencher.bench_local(|| {
        let treebank = Treebank::from_path(Path::new("examples/text_fic_2010.conll"));
        let count = black_box(treebank.trees(false).count());
        black_box(count);
    });
}
*/
...
fn tree_iter_ordered_multi(bencher: Bencher) {
    ...
    bencher.bench_local(|| {
        let count = black_box(treebank.clone().trees(true).count());
        black_box(count);
    });
}
 */
...
fn tree_iter_unordered_multi(bencher: Bencher) {
    let treebank = Treebank::from_glob("/Volumes/Corpora/COHA/conll/text_*_19[0-5]0.conllu.gz").unwrap();
    bencher.bench_local(|| {
        let count = black_box(treebank.clone().trees(false).count());
        black_box(count);
    });
}
 */
...
fn match_iter_ordered_single(bencher: Bencher) {
    ...
    bencher.bench_local(|| {
        let treebank = Treebank::from_path(Path::new("examples/text_fic_2010.conll"));
        let count = black_box(treebank.search(pattern.clone(), true).unwrap().count());
        black_box(count);
    });
}
*/
...
fn match_iter_unordered_single(bencher: Bencher) {
    ...
    bencher.bench_local(|| {
        let treebank = Treebank::from_path(Path::new("examples/text_fic_2010.conll"));
        let count = black_box(treebank.search(pattern.clone(), false).unwrap().count());
        black_box(count);
    });
}
*/
